...
func generateHTML(w http.ResponseWriter, data interface{}, filnames ...string) {
	...
	//var templates = template.Must(template.ParseFiles(files...))と記述して、
...
//酒井潤の講義 89 のハンズオンであった、makeHandler()関数と同じ役割だよ
//つまり、ラッパーだよ
//ラッパーは、todoEdit 関数（もともとの機能）を包んで、代わりに矢面に立ってくれていますよね。
//さらに、todoEdit 関数を変えないで、もっと使えるやつ（目的に合致したやつ）にしてくれています。
func parseURL(fn func(http.ResponseWriter, *http.Request, int)) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		//fmt.Println(q)
		//[/todos/edit/1 edit 1] のような配列となっているよ
		...
		//func に渡して、関数を実行します
		//parseURL 関数内で、1 などの ID を取得して、最終的には todoEdit を実行します
		fn(w, r, qi)
	}
}
